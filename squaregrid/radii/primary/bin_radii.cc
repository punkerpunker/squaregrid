...
DEFINE_int32(radii_count, 6, "Number of radii to calculate");
...
int main(int argc, char **argv) {
	...
	try {
		pqxx::connection db_conn(
				"host=127.0.0.1 port=5432 dbname=mldata user=marketinglogic password=tttBBB777");
		std::vector<grids::GridMetadata> grids = grids::LoadGridMetadata(&db_conn);
		LOG(INFO) << "LUL " << FLAGS_data_query;
